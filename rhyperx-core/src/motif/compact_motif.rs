pub mod inner {
    ...
    /// Hyperedge motif encoded as a flat bit-vector. Useful to enable simd optimization on small
    /// graphlets operations (e.g. fingerprint computation).
    ///
    ...
    ///
    /// Hyperedges are indexed starting from size 1 to n, in ascending order.
    ///
    /// This data struct can store motifs up to order 8, since it heavily relies on tables computed at compile time to enable fast operations on
    /// motifs. The combinatorial explosion of the number of hyperedges makes it impractical to support
    /// larger motifs, as the tables would become too large to compile and store in memory.
    ///
    /// # Type parameters
    ...
    /// # Example
    /// ```
    /// use rhyperx_core::compact_motif;
    /// use rhyperx_core::CompactMotif;
    ///
    ...
    mod __ {
            ...
            CompactMotif<tm_type, N, M, AM, P>
        {
            ...
            pub(crate) const ADJ: [Self; N] = const {
            ...
            pub(crate) const NODE_MAP: [BinStore<u8, 1>; M] = const {
                let mut rv = [BinStore::<u8, 1>::ZERO; M];
                iter_hyperedges!(N, 1..=N, |edge, edge_size, edge_idx| {
                    let mut i = 0;
                    while i < edge_size {
                        rv[edge_idx].set_bit(edge[i]);
                        i = i + 1;
                    }
                });
                rv
            ...
            pub(crate) const FULL_OVERLAPS: [Self; M] = const {
            ...
            pub(crate) const PART_OVERLAPS: [Self; M] = const {
                let adj = Self::ADJ;
            ...
            pub const RELABELING_MAP: [[usize; M]; P] = const {
                ...
                while i < factorial(N) {
                    ...
                    while j < M {
                        ...
                        while !old_nodes.is_empty() {
                            let old_node = old_nodes.pop() as usize;
            ...
            CompactMotif<tm_type, N, M, AM, P>
        {
            ...
            pub const fn add_edge_with_nodes_unchecked<const NN: usize>(
                &mut self,
                nodes: [u8; NN],
            ) {
                let mut i = 0;
            ...
            pub const fn remove_edge_with_nodes_unchecked(&mut self, nodes: &[u8]) -> bool {
                let mut i = 0;
                let mut set = BinStore::<u8, 1>::ZERO;
                while i < nodes.len() {
                    let node = nodes[i] as usize;
            ...
            pub fn iter_edges_ids(&self) -> CompactMotifEdgeIter<tm_type, AM> {
            ...
            pub fn is_connected(&self) -> bool {
                ...
                let mut covered_nodes = BinStore::<u8, 1>::ZERO;
                for e in self.iter_edges_ids() {
                    covered_nodes.bitor_assign(&Self::NODE_MAP[e]);
                }
                let full_mask = BinStore::<u8, 1>::ONE.shift_left(N).sub_raw(1);
                // (1 << N) - 1;
                if covered_nodes != full_mask {
                    return false;
                }

                let first_edge = match self.iter_edges_ids().next() {
                    Some(e) => e,
                    None => return false,
                };
            ...
            pub fn to_vec(&self) -> Vec<Vec<usize>> {
                ...
                for e in self.iter_edges_ids() {
                    ...
                    while !node_bits.is_empty() {
                        let n = node_bits.pop() as usize;
                        nodes.push(n);
                    }
                    edges.push(nodes);
                }
        ...
        impl<const N: usize, const M: usize, const AM: usize, const P: usize> Default
            for CompactMotif<tm_type, N, M, AM, P>
        {
            fn default() -> Self {
                Self::EMPTY
...
/// Expands to the [`crate::motif::inner::CompactMotif`] type alias for `N` nodes (`2 ≤ N ≤ 8`).
///
/// The `CompactMotif` data structure relies on several compile-time const generics
/// (bit-packing type, node count, edge mask, storage size, and factorial permutations)
/// to maximize memory efficiency. This macro serves as a convenient type-level helper
/// to avoid having to spell out the complex const parameters manually.
///
/// # Examples
///
/// Basic usage defining a variable with the generated type:
///
/// ```
/// use rhyperx_core::motif::CompactMotif;
///
/// type Motif4 = CompactMotif!(4);
/// let motif: Motif4 = Default::default();
/// assert_eq!(motif.edge_count(), 0);
/// ```
///
/// Using it directly as a type annotation in a function signature:
///
/// ```
/// use rhyperx_core::motif::CompactMotif;
///
/// fn process_motif_3(motif: &CompactMotif!(3)) {
///     assert_eq!(motif.edge_count(), 0);
/// }
/// ```
#[macro_export]
macro_rules! CompactMotif {
    (2) => {
        $crate::motif::inner::CompactMotif::<u8, 2, { (1 << 2) - 1 }, 1, 2>
    };
    (3) => {
        $crate::motif::inner::CompactMotif::<
            u8,
            3,
            { (1 << 3) - 1 },
...
/// Instantiates an empty [`crate::motif::inner::CompactMotif`] value for `N` nodes (`2 ≤ N ≤ 8`).
///
/// This macro expands to the static `EMPTY` constant on the matching `CompactMotif` type
/// variant for a given size `N`.
///
/// # Examples
///
/// Constructing an empty motif for 4 nodes:
///
/// ```
/// use rhyperx_core::motif::CompactMotif;
/// use rhyperx_core::compact_motif;
///
/// let motif: CompactMotif!(4) = compact_motif!(4);
/// assert_eq!(motif.edge_count(), 0);
/// ```
///
/// Passing an empty motif into a function expecting a specific size:
///
/// ```
/// use rhyperx_core::compact_motif;
///
/// // Instantiates the 3-node empty motif directly
/// let empty_graph = compact_motif!(3);
/// assert!(empty_graph.is_empty());
/// ```
#[macro_export]
macro_rules! compact_motif {
    (2) => {
        <$crate::motif::CompactMotif!(2)>::EMPTY
    };
    (3) => {
        <$crate::motif::CompactMotif!(3)>::EMPTY
    };
    (4) => {
        <$crate::motif::CompactMotif!(4)>::EMPTY
    };
    (5) => {
        <$crate::motif::CompactMotif!(5)>::EMPTY
    };
    (6) => {
        <$crate::motif::CompactMotif!(6)>::EMPTY
    };
    (7) => {
        <$crate::motif::CompactMotif!(7)>::EMPTY
    };
    (8) => {
        <$crate::motif::CompactMotif!(8)>::EMPTY
    };
    ($n:expr) => {
...
pub use CompactMotif;
pub use compact_motif;
